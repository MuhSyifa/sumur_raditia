...
<style>.ui-widget-header{background:none;border:none;}</style>
		...
		<?php
		$aksi="modul/mod_galeri_pengalaman/aksi_galeri_pengalaman.php";
		switch($_GET[act]){
			default:
		?>
		...
		<article style="min-width:535px;" class="module width_3_quarter">
			<header><h3 class="tabs_involved">Header</h3>
			...
			<?php 	
				$no=1;
				$gallery = mysql_query("SELECT * FROM galeri_pengalaman ORDER BY id ASC");
				...
				<tr>  
    				...
    				<td style="text-align:center"><a href="<?php echo"?module=galeri_pengalaman&act=edit&id=$s[id]";?>"><input type="image" src="images/icn_edit.png" title="Edit"></a>&nbsp;&nbsp;&nbsp;&nbsp;</td> 
				</tr> 
			<?php $no++; } ?>
		...
		<?php break; 
		case"edit":
			$galleryshow = mysql_query("SELECT * FROM galeri_pengalaman WHERE id='$_GET[id]'");
				$g=mysql_fetch_array($galleryshow);
		
		?>
		
		<article class="module width_quarter">
			 <header><h3 class="tabs_involved">Edit Header</h3>
				
				<input style="float:right; margin: -27px 10px 0px 0px;" type='button'  class='tombol' value='Back' onclick='self.history.back()'>
				
			</header>
			 <form method='POST' enctype='multipart/form-data' action='modul/mod_galeri_pengalaman/aksi_galeri_pengalaman.php?module=galeri_pengalaman&act=update'>
