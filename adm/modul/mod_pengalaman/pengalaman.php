...
<!-- /TinyMCE -->
		...
		<?php 	$aksi="modul/mod_pengalaman/aksi_pengalaman.php";
				$review = mysql_query("SELECT * FROM modul WHERE id_modul='5' ");
				$w=mysql_fetch_array($review);
				?>
		
		<article class="module width_full">
			<header><h3>Edit Pengalaman</h3></header>
			<form method='POST' enctype='multipart/form-data' action='<?php echo "$aksi"; ?>?module=pengalaman&act=update'>
				<input type='hidden' name='id' value='<?php echo"$w[id_modul]" ?>'>
				<div class="module_content">
							<textarea name="isi" rows="12" style="max-heigth:300px;"><?php echo"$w[static_content]" ?></textarea>
						<div class="clear"></div>
				</div>
			<footer>
